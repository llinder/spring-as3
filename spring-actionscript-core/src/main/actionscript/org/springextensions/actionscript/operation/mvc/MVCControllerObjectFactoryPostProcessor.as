...
package org.springextensions.actionscript.operation.mvc {
	...
	public class MVCControllerObjectFactoryPostProcessor implements IObjectFactoryPostProcessor {
		 ...
		 * @param objectFactory The specified <code>IConfigurableListableObjectFactory</code> instance.
		 */
		public function addRouteEventsMetaDataPostProcessor(objectFactory:IConfigurableListableObjectFactory):void {
			Assert.notNull(objectFactory, "the objectFactory argument must not be null")
			if (objectFactory.getObjectNamesForType(MVCRouteEventsMetaDataProcessor).length < 1) {
				...
				objectFactory.registerObjectDefinition(METADATAPROCESSOR_OBJECT_NAME, objectDefinition);
			}
		}
		 ...
		 */
		public function registerCommands(objectFactory:IConfigurableListableObjectFactory, controller:IController):void {
			Assert.notNull(objectFactory, "the objectFactory argument must not be null");
			Assert.notNull(controller, "the controller argument must not be null");
			for each (var key:String in objectFactory.objectDefinitionNames) {
				var def:IObjectDefinition = IObjectDefinition(objectFactory.objectDefinitions[key]);
