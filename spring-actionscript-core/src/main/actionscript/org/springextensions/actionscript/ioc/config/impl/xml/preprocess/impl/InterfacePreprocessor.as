...
package org.springextensions.actionscript.ioc.config.impl.xml.preprocess.impl {
	...
	public class InterfacePreprocessor implements IXMLObjectDefinitionsPreprocessor, IApplicationDomainAware {
		private static const AT_CHAR:String = "@";
		private static const CLASS_ATTRIBUTE_IDENT:String = "@class";
		private static const NAME_ATTRIBUTE_IDENT:String = "@name";
		...
		public function preprocess(xml:XML):XML {
			...
			for each (node in nodes) {
				var className:String = node[CLASS_ATTRIBUTE_IDENT];
				...
				for each (var objectNode:XML in objects) {
					...
					for each (var child:XML in node.children()) {
						...
						var nameAttributeValue:String = child[NAME_ATTRIBUTE_IDENT];
					...
					for each (var attr:XML in node.attributes()) {
						...
						var objectHasAttribute:Boolean = (objectNode[AT_CHAR + attributeName] != undefined);

						if (!objectHasAttribute) {
							objectNode[AT_CHAR + attributeName] = node[AT_CHAR + attributeName].copy();
						}
					}
				}
			}
		...
		protected function hasChildElementWithName(node:XML, childElementName:String, namePropertyValue:String):Boolean {
			...
			for each (var child:XML in children) {
				if ((child.localName().toString() == childElementName) && (child[NAME_ATTRIBUTE_IDENT] == namePropertyValue)) {
					return true;
				}
			}
			return false;
