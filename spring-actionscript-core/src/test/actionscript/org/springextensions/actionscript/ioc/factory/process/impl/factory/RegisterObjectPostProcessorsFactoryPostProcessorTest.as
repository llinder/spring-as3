...
package org.springextensions.actionscript.ioc.factory.process.impl.factory {
	...
	public class RegisterObjectPostProcessorsFactoryPostProcessorTest {
		...
		public function testPostProcessObjectFactory():void {
			...
			mock(objectDefinitionRegistry).method("getObjectNamesForType").args(anything()).returns(names).once();
			mock(objectFactory).getter("objectDefinitionRegistry").returns(objectDefinitionRegistry).once();
			mock(objectFactory).method("getObject").args("processor1").returns(processor1).once();
			mock(objectFactory).method("getObject").args("processor2").returns(processor2).once();
			mock(objectFactory).method("getObject").args("processor3").returns(processor3).once();

			var processor:RegisterObjectPostProcessorsFactoryPostProcessor = new RegisterObjectPostProcessorsFactoryPostProcessor();
