...
package org.springextensions.actionscript.ioc.config.impl.metadata {
	...
	public class ConfigurationClassScannerTest {
		...
		public function testScan():void {
			...
			assertEquals("objectName1", RuntimeObjectReference(propDef.value).objectName);

			propDef = definition.getPropertyDefinitionByName("someOtherProperty");
			assertNotNull(propDef);
			assertTrue(propDef.value is RuntimeObjectReference);
			assertEquals("objectName2", RuntimeObjectReference(propDef.value).objectName);
			...
			assertTrue(methDef.arguments[0] is RuntimeObjectReference);
			assertEquals("objectName1", RuntimeObjectReference(methDef.arguments[0]).objectName);
			...
			assertEquals("objectName1", RuntimeObjectReference(definition.constructorArguments[0]).objectName);
