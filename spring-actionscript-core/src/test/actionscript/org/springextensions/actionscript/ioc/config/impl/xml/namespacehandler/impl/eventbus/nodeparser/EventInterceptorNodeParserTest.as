...
package org.springextensions.actionscript.ioc.config.impl.xml.namespacehandler.impl.eventbus.nodeparser {
	 ...
	 * @author Roland Zwaga
	 */
	public class EventInterceptorNodeParserTest extends AbstractNodeParserTest {
		public static const simpleEventInterceptorXML:XML = new XML('<eventbus:event-interceptor xmlns:eventbus="http://www.springactionscript.org/schema/eventbus" instance="test">' + //
			'<eventbus:interception-configuration event-name="event1"/>' + //
			'</eventbus:event-interceptor>');

		public static const simpleEventInterceptorWithTopicXML:XML = new XML('<eventbus:event-interceptor xmlns:eventbus="http://www.springactionscript.org/schema/eventbus" instance="test">' + //
			'<eventbus:interception-configuration event-name="event1" topics="topic"/>' + //
			'</eventbus:event-interceptor>');

		public static const simpleEventInterceptorWithTopicAndPropertiesXML:XML = new XML('<eventbus:event-interceptor xmlns:eventbus="http://www.springactionscript.org/schema/eventbus" instance="test">' + //
			'<eventbus:interception-configuration  event-name="event1" topics="topic" topic-properties="prop1,prop2"/>' + //
			'</eventbus:event-interceptor>');

		public static const simpleEventInterceptorWithClassAndMultipleMethodHandlersXML:XML = new XML('<eventbus:event-interceptor xmlns:eventbus="http://www.springactionscript.org/schema/eventbus" instance="test">' + //
			'<eventbus:interception-configuration event-name="event1" topics="topic" topic-properties="prop1, prop2"/>' + //
			'<eventbus:interception-configuration event-class="flash.events.Event" topic-properties="topic" />' + //
			'</eventbus:event-interceptor>');

		/**
		 * Creates a new <code>EventInterceptorNodeParserTest</code> instance.
		 */
		public function EventInterceptorNodeParserTest() {
			super();
		}
		...
		public function testParseWithSimpleXML():void {
			...
			var parser:EventInterceptorNodeParser = new EventInterceptorNodeParser(objectDefinitionRegistry, eventBusUserRegistry, ApplicationDomain.currentDomain);
			parser.parse(simpleEventInterceptorXML, xmlParser);

			verify(objectDefinitionRegistry);
			verify(objectDefinition);
			assertEquals(1, configs.length);
			assertTrue(configs[0] is EventInterceptorCustomConfigurator);
			var config:EventInterceptorCustomConfigurator = EventInterceptorCustomConfigurator(configs[0]);
		...
		public function testParseWithSimpleXMLWithClassNameAndMultipleMethodConfigs():void {
			...
			assertTrue(configs[0] is EventInterceptorCustomConfigurator);
			assertTrue(configs[1] is EventInterceptorCustomConfigurator);
			...
			config = EventInterceptorCustomConfigurator(configs[1]);
			assertStrictlyEquals(eventBusUserRegistry, config.eventBusUserRegistry);
